...
import { getCompletedTrips } from "@/api/driver"
...
export function CompletedTrips() {
    ...
    const [trips, setTrips] = useState(completedTripsData) // Start with mock data
    ...
    useEffect(() => {
        async function fetchTrips() {
            try {
                setLoading(true)
                const response = await getCompletedTrips(userId)
                ...
                toast({
                    title: "Error",
                    description: "Could not load completed trips",
                    variant: "destructive",
                })
            } finally {
                setLoading(false)
    ...
    // Use trips from API data instead of completedTripsData
    const filteredTrips = trips.filter((trip) => {
        ...
        const matchesDate = dateFilter ? trip.date.includes(dateFilter) : true
        ...
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>View your trip history and details</CardDescription>
                </CardHeader>
                <CardContent>
                                    ...
                                    {filteredTrips.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                                                {trips.length === 0 ? "No completed trips found" : "No trips found matching your criteria"}
                                            </TableCell>
                                        </TableRow>
                                    ) : (
